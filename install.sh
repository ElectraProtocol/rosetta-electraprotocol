...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for ElectraProtocol/rosetta-electraprotocol
...
execute() {
  ...
  docker tag "rosetta-electraprotocol:${TAG}" "rosetta-electraprotocol:latest"
  log_info "loaded rosetta-electraprotocol:${TAG} and tagged as rosetta-electraprotocol:latest"
  rm -rf "${tmpdir}"
...
BINARY=rosetta-electraprotocol
FORMAT=tar.gz
OWNER=coinbase
REPO="rosetta-electraprotocol"
PREFIX="$OWNER/$REPO"
