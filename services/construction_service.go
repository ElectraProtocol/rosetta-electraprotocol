...
func (s *ConstructionAPIService) estimateSize(operations []*types.Operation) float64 {
	size := electraprotocol.TransactionOverhead
	for _, operation := range operations {
		switch operation.Type {
		case electraprotocol.InputOpType:
			size += electraprotocol.InputSize
		case electraprotocol.OutputOpType:
			size += electraprotocol.OutputOverhead
			addr, err := btcutil.DecodeAddress(operation.Account.Address, s.config.Params)
			if err != nil {
				size += electraprotocol.P2PKHScriptPubkeySize
				continue
			}
...
func (s *ConstructionAPIService) ConstructionPreprocess(
	...
	descriptions := &parser.Descriptions{
		OperationDescriptions: []*parser.OperationDescription{
			{
				Type: electraprotocol.InputOpType,
...
func (s *ConstructionAPIService) ConstructionMetadata(
	...
	if feePerKB < electraprotocol.MinFeeRate {
		feePerKB = electraprotocol.MinFeeRate
	}

	// Calculated the estimated fee in Satoshis
	satoshisPerB := (feePerKB * float64(electraprotocol.SatoshisInBitcoin)) / bytesInKb
...
func (s *ConstructionAPIService) ConstructionPayloads(
	...
	descriptions := &parser.Descriptions{
		OperationDescriptions: []*parser.OperationDescription{
			...
			{
				Type: electraprotocol.OutputOpType,
				Account: &parser.AccountDescription{
					Exists: true,
				},
				Amount: &parser.AmountDescription{
	...
	for _, input := range matches[0].Operations {
		...
		transactionHash, index, err := electraprotocol.ParseCoinIdentifier(input.CoinChange.CoinIdentifier)
		if err != nil {
	...
	for i := range tx.TxIn {
		...
		class, _, err := electraprotocol.ParseSingleAddress(s.config.Params, script)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionCombine(
	...
	if err := json.Unmarshal(decodedTx, &unsigned); err != nil {
		return nil, wrapErr(
			ErrUnableToParseIntermediateResult,
			fmt.Errorf("%w unable to unmarshal electraprotocol transaction", err),
		)
	}
	...
	for i := range tx.TxIn {
		...
		class, _, err := electraprotocol.ParseSingleAddress(s.config.Params, decodedScript)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionHash(
	...
	if err := json.Unmarshal(decodedTx, &signed); err != nil {
		return nil, wrapErr(
			ErrUnableToParseIntermediateResult,
			fmt.Errorf("%w unable to unmarshal signed electraprotocol transaction", err),
		)
	}
...
func (s *ConstructionAPIService) parseUnsignedTransaction(
	...
	for i, output := range tx.TxOut {
		networkIndex := int64(i)
		_, addr, err := electraprotocol.ParseSingleAddress(s.config.Params, output.PkScript)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) parseSignedTransaction(
	...
	for i, input := range tx.TxIn {
		...
		_, addr, err := electraprotocol.ParseSingleAddress(s.config.Params, pkScript.Script())
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionSubmit(
	...
	if err != nil {
		return nil, wrapErr(ErrXepd, fmt.Errorf("%w unable to submit transaction", err))
	}
