...
GO_PACKAGES=./services/... ./indexer/... ./electraprotocol/... ./configuration/...
...
build:
	docker build -t rosetta-electraprotocol:latest https://github.com/ElectraProtocol/rosetta-electraprotocol.git

build-local:
	docker build -t rosetta-electraprotocol:latest .

build-release:
	# make sure to always set version with vX.X.X
	docker build -t rosetta-electraprotocol:$(version) .;
	docker save rosetta-electraprotocol:$(version) | gzip > rosetta-electraprotocol-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/electraprotocol-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 8333:8333 rosetta-electraprotocol:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-electraprotocol:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/electraprotocol-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 18333:18333 rosetta-electraprotocol:latest
