...
import (
	...
	mocks "github.com/ElectraProtocol/rosetta-electraprotocol/mocks/indexer"
...
func TestIndexer_Transactions(t *testing.T) {
	...
	type coinBankEntry struct {
		Script  *electraprotocol.ScriptPubKey
		Coin    *types.Coin
	...
	for i := int64(0); i <= 1000; i++ {
		...
		for j := 0; j < 5; j++ {
			...
			scriptPubKey := &electraprotocol.ScriptPubKey{
				ASM: coinIdentifier,
			}
				...
				Operations: []*types.Operation{
					{
						...
						Status: types.String(electraprotocol.SuccessStatus),
	...
	go func() {
		for {
			...
			if currBlock.BlockIdentifier.Index == 1000 {
				...
				allCoins := []*types.Coin{}
				expectedPubKeys := []*electraprotocol.ScriptPubKey{}
